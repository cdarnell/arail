...
**New function** in the canonical Buddy module
`src/arail/agents/_builtin_buddy.py` (single source of truth post-repave —
see *Buddy de-duplication* below). The runtime PKB copy at
`lab/pkb/agents/buddy/buddy.py` becomes a thin shim that re-exports from
the package, so a single edit lands the watcher everywhere it can run:
...
- A *third* place where the lab cares about modes (after `LAB_MODE`,
  ...
  `is_airgapped()`.
- (Removed — the parallel Buddy duplication is repaved in this sprint.
  See *Buddy de-duplication* below. No stub learning is filed; the
  repave itself is the resolution.)
...
- The Buddy duplication (canonical `_builtin_buddy.py` vs. PKB
...
## Buddy de-duplication

Repaving the parallel-Buddy debt is in-scope for this sprint. The
airgap watcher must land in one file, not two — and the cleaner end
state ships now.

### B.1 — Canonical path

**Canonical: `src/arail/agents/_builtin_buddy.py`** (the in-package
module).

Rationale (one line each):

- Already shipped in git; the PKB copy at `lab/pkb/agents/buddy/buddy.py`
  is **not tracked** (`.gitignore` line 42: `lab/pkb/agents/`). Today's
  drift is workstation-only — the canonical-in-repo file IS
  `_builtin_buddy.py`. Repave aligns reality with intent.
- The loader's existing `_builtin_fallback("buddy")` already imports
  this module; no loader churn required.
- `src/arail/agents/forge.py` and `docs/agents.md` already point new
  contributors at `_builtin_buddy.py` as the reference implementation.
- `src/arail/agents/builtin_seed.py:490` already treats it as the seed
  source (`shutil.copy(_builtin_buddy.py, buddy.py)`).

We are *not* moving the canonical body to `lab/pkb/agents/buddy/buddy.py`.
That direction would force `pyproject.toml` package-data shenanigans to
keep the body shipped with the wheel, would invert the loader's fallback
semantics, and would break the `arail.agents._builtin_buddy` import that
`tests/test_buddy_suggesters.py` and `forge.py` already use.

### B.2 — What happens to `lab/pkb/agents/buddy/buddy.py`

It becomes a **thin re-export shim**, auto-written by
`builtin_seed.ensure_buddy_folder()` on first boot. The current
`shutil.copy(_builtin_buddy.py, buddy.py)` is replaced with a
templated shim write.

**Exact shim contents** (≤ 30 lines, written by `ensure_buddy_folder`):

```python
"""Buddy — PKB shim.

This file is auto-generated by builtin_seed.ensure_buddy_folder. The
canonical implementation lives in the installed package at
``src/arail/agents/_builtin_buddy.py``. The shim re-exports the
``buddy`` singleton (and the helpers tests reach for) so the loader
finds an importable agent here without us shipping a forked copy
that drifts.

To fork Buddy into a custom personality, replace this file with a
full module body — copy `_builtin_buddy.py` as a starting point, edit
NAME / SYSTEM_PROMPT / WATCHERS / SUGGESTERS, and the loader will
prefer this PKB copy over the package fallback.

DO NOT EDIT THIS SHIM. It will be regenerated if missing.
"""

from arail.agents._builtin_buddy import (  # noqa: F401
    buddy,
    BuddyAgent,
    Observation,
    WATCHERS,
...
The shim is *idempotent* in `ensure_buddy_folder`: if `buddy.py`
exists and its first non-blank line starts with `"""Buddy — PKB shim."""`,
it's our shim — leave it alone (or refresh on header-version bump).
If the first line is anything else, the user has forked — also leave
it alone. Old workstations with a stale full-body copy fall into the
"forked" branch by accident; addressed in B.7.

### B.3 — Loader behavior

`src/arail/agents/loader.py` is **unchanged in code**, with one comment
update. The existing chain still applies:

1. `load_one("buddy")` calls `_seed_if_shipped("buddy")` →
   `ensure_buddy_folder()` (now writes a shim if missing).
2. Imports `lab/pkb/agents/buddy/buddy.py` dynamically — the shim
   re-imports the canonical, so `getattr(module, "buddy", None)`
   returns the same singleton instance the package exports.
3. If the PKB import fails (e.g. user's edit broke the file), falls
   back to `_builtin_fallback("buddy")` → direct import of
   `arail.agents._builtin_buddy`. **This fallback survives** because
   the shim is just a thin import; if the user breaks it (or wipes
   their PKB), Buddy still loads from the package.

A user with a missing PKB Buddy folder gets:
- `ensure_buddy_folder()` re-creates folder + writes shim →
- Loader imports shim → gets canonical singleton →
- Identical to the package-fallback path. No user-visible difference.

The fallback's purpose ("keep Buddy alive when PKB copy is broken")
is fully preserved.

### B.4 — Direct importers (grep results, 2026-05-05)

```
$ grep -rn "_builtin_buddy" --include="*.py" .
src/arail/agents/buddy.py:29        from arail.agents._builtin_buddy import buddy as _fallback_buddy
src/arail/agents/forge.py:198       # comment ("matches _builtin_buddy.py")
src/arail/agents/forge.py:209       # docstring ref
src/arail/agents/forge.py:439       # docstring ref
src/arail/agents/builtin_seed.py:4  # docstring mention
src/arail/agents/builtin_seed.py:489-490  shutil.copy(_builtin_buddy.py, buddy.py)
tests/test_buddy_suggesters.py:19   from arail.agents import _builtin_buddy as buddy_mod
```

No `from lab.pkb.agents.buddy import …` in tree — the PKB copy is
loaded only via the dynamic-import path in `loader.py:_import_from_path`.

| Site | Action |
|---|---|
| `src/arail/agents/buddy.py:29` | **leave alone.** Already references the canonical. |
| `src/arail/agents/forge.py:198,209,439` | **leave alone.** Comment / docstring references; the canonical name is unchanged. |
| `src/arail/agents/builtin_seed.py:4` | **leave alone.** Docstring already says canonical lives in `_builtin_buddy.py`. |
| `src/arail/agents/builtin_seed.py:489-490` | **edit (B.2).** Replace `shutil.copy(builtin, buddy_py)` with the shim-template write. Builder writes a `_BUDDY_PKB_SHIM` constant near the other templates and writes that string instead. |
| `tests/test_buddy_suggesters.py:19` | **leave alone.** Imports the canonical — same module, same suggesters. Tests pass unchanged. |
| `src/arail/portal/templates/agents.html:225` | **leave alone.** Already links to `_builtin_buddy.py` on GitHub, with a tooltip pointing at the editable PKB path. Copy stays accurate. |
| `docs/agents.md:32,92,98,101,189,514,528` | Update **only the line that says "Copy `src/arail/agents/_builtin_buddy.py` → a new file"** to clarify that the PKB shim is the seed default; users replace the shim with a copy of `_builtin_buddy.py` to fork. (Builder edits docs/agents.md once.) |
| `docs/agents-explained.md:85,239` | **leave alone.** References point to canonical — accurate. |

### B.5 — Test fallout

One test file references `_builtin_buddy` directly:

- `tests/test_buddy_suggesters.py:19` — `from arail.agents import _builtin_buddy as buddy_mod`. **Stays.** It tests the canonical module's suggester functions; that's correct after the repave (canonical is `_builtin_buddy`).

A new test must verify the shim:

- `tests/test_builtin_seed_buddy_shim.py` (**new**, alongside step B.2) —
  - With a clean tmp PKB root, call `ensure_buddy_folder` → assert
    `buddy.py` exists, first non-blank line equals `'"""Buddy — PKB shim."""'`.
  - Assert the shim file is < 60 lines (regression guard against
    accidental full-body copy).
  - Import the shim file via `importlib.util.spec_from_file_location`,
    assert `module.buddy is arail.agents._builtin_buddy.buddy`
    (identity, not equality — proves the shim doesn't fork the
    singleton).
  - With an existing forked file (header line ≠ shim sentinel), call
    `ensure_buddy_folder` → assert the file is **not** rewritten
    (idempotency for forked users).

This new test is added as row #28 in §13.

### B.6 — Watcher placement

`_watch_airgap_events` lives in **`src/arail/agents/_builtin_buddy.py`
only.** It is registered in the canonical `WATCHERS` list; the shim
re-exports `WATCHERS` so the loader sees the same list whether it
imported the shim or fell back to the package directly. There is no
parallel `_watch_airgap_events` in `lab/pkb/agents/buddy/buddy.py` —
the file is a shim, not a copy.

### B.7 — Ordering invariant

The repave **must precede the watcher** so the builder writes the
watcher exactly once.

Implementation order (replaces the prior §14 list):

1. Layer 1 — single source of truth. `src/arail/airgap.py` +
   `tests/test_airgap_helpers.py`. Green before moving on.
2. Layer 2 — egress guard. `src/arail/egress.py` +
   `tests/test_egress_guard.py` + `tests/conftest.py` autouse fixture.
3. **Buddy repave (NEW STEP).** Files 13b (builtin_seed.py: replace
   `shutil.copy` with shim template), 13c (delete the workstation's
   `lab/pkb/agents/buddy/buddy.py` so the next boot re-seeds), and
   the new `tests/test_builtin_seed_buddy_shim.py`. Confirm
   `./arail start` materializes the shim and Buddy still loads.
4. Consolidate `LAB_MODE` call sites. Files 6, 9, 10, 11, 12, 15.
   Run regression tests after each.
5. API + modal. File 7 (`/api/airgap/status` route), 18, 19, 20, 21.
6. **Buddy watcher (now single-file).** File 13a only:
   `src/arail/agents/_builtin_buddy.py` gets `_watch_airgap_events()`,
   `LAB_INTERNET_ENABLED` is dropped, HF-papers fetch is gated on
   `is_airgapped()`. Plus `tests/test_buddy_airgap_watcher.py`.
7. Wire-in to portal startup. `portal/app.py` startup +
   `agents/loader.py` first-line `install_guard()`. Smoke-run.
8. Docs. Files 22, 23. Verbatim copy from §11.
9. Audit comments. File 16 (`# noqa-airgap: localhost-only`).
10. Learnings. File 27 (the asyncio.create_task contextvars stub
    survives; the Buddy-duplication learning is dropped — repaved,
    not learned-from).
11. End-to-end manual demo. Per VISION §3 / PLAN.md.

The hard rule: **step 3 (repave) lands before step 6 (watcher)**.
If the builder writes the watcher first, they will end up either
editing two files (defeating the sprint goal) or having to delete
the duplicate after the fact (more diff churn, more risk).

### B.8 — Risk + rollback

**Worst case:** the shim re-export misses a name that an existing
runtime caller reaches for via `lab.pkb.agents.buddy.buddy.<thing>`.
The dynamically-imported PKB module would lack the attribute and the
caller fails at runtime.

**Detection:** sprint-test boot via `./arail start` after step 3;
exercise Buddy chat, the dashboard activity tab, and `/agents`. Any
`AttributeError` involving `buddy_mod.<x>` is the signal.

**Mitigation in shim:** the shim re-exports the union of names the
canonical module exposes that any in-tree caller currently reaches
for (B.2 list: `buddy`, `BuddyAgent`, `Observation`, `WATCHERS`,
`SUGGESTERS`, `NAME`, `EMOJI`, `SYSTEM_PROMPT`). If a caller
reaches deeper (private helpers like `_suggest_phase_action`),
they are accessing the canonical module by its `arail.agents._builtin_buddy`
import path already (see test_buddy_suggesters.py), not by re-import
through the PKB module — so the shim's narrower surface is fine.

**Rollback recipe** (if a fresh install breaks Buddy after this
ships and we need to revert without un-shipping the airgap watcher):

1. In `builtin_seed.ensure_buddy_folder`, flip the body write back
   to `shutil.copy(builtin, buddy_py)` — one-line revert.
2. The watcher is in `_builtin_buddy.py` (canonical) so it survives
   the rollback; the PKB copy is now a fresh full-body clone with
   the watcher included by virtue of having been copied from the
   canonical.
3. No data loss: `state.json`, `decisions.md`, `dreams/` are
   untouched by the repave (only `buddy.py` is rewritten).

**Net additional risk vs. the prior "edit both files in lockstep"
plan:** one new failure mode (shim re-export gap), mitigated by an
explicit acceptance test in the repave step. Lower long-run risk
than maintaining two parallel files.
...
| 13d | `src/arail/agents/loader.py` | edit (one-line clarification) | comment update only — the existing `_SHIPPED` fallback chain survives untouched; document that PKB Buddy is now a shim by convention but the loader still imports it the same way. |
| 13e | `tests/test_buddy_suggesters.py` | leave | already imports `arail.agents._builtin_buddy` (the canonical post-repave) — no change needed. Sprint-internal verification only. |
