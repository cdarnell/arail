...
|---|---|---|---|---|
| 1 | `src/arail/portal/scheduler.py`, `src/arail/portal/app.py` | NEW scheduler.py — inference semaphore + fast-path metrics. Wire `fastpath_meter` middleware after `onboarding_gate` in app.py. Metrics-only; no behavior change to chat. | None (QA phase) | — |
...
| 7 | `src/arail/portal/app.py` | Boot-scan task inserted at startup (hybrid mode only, after dream daemon block). | None (QA phase) | — |
...
**Plan:** `if _lab_mode() == "hybrid": asyncio.create_task(_boot_security_scan())` at end of startup function.
