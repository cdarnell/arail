"""Wrapper for tests/cli/install_driver.sh (WP7 gates: T24-T28, F5-F7,
F21, F22, F28, F32).

Same pattern as tests/test_cli_root_start.py wrapping
tests/cli/root_start_driver.sh. Uses real git repos (a fake checkout
tracking a local bare "remote") — no network required, no dependency on
this machine's own git config beyond a working `git` binary. Self-skips
(driver prints "SKIP:") when no usable venv is found.
"""
...
DRIVER = REPO_ROOT / "tests" / "cli" / "install_driver.sh"
...
def test_install_driver_scenarios():
