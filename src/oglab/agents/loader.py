...
(``src/oglab/agents/_builtin_<id>.py``) that the loader falls
...
from oglab.skills_loader import parse_frontmatter

log = logging.getLogger(__name__)


# Agents with a bundled builtin in src/oglab/agents/_builtin_<id>.py.
...
def _builtin_fallback(agent_id: str) -> Optional[Any]:
    ...
    try:
        module = __import__(
            f"oglab.agents._builtin_{agent_id}",
            fromlist=[agent_id],
        )
    except Exception as e:  # noqa: BLE001
...
def load_one(agent_id: str, pkb_root: Path | None = None) -> Optional[Any]:
    ...
    unique = f"oglab.agents._folder_{agent_id}"
...
def start_all_auto(agents: Dict[str, Any], pkb_root: Path | None = None) -> None:
    ...
    """
    from oglab.agents.dream_daemon import register as register_dream
