"""oglab.experiments.autoresearch — The autonomous tuning loop.
...
Safety rails (non-negotiable, same for both backends):
    ...
    - OGLAB_AUTORESEARCH_ENABLED env var must be set.
...
def _build_commit_message(
    *,
    ...
    lines.append("")
    lines.append(
        "Produced by oglab.experiments.autoresearch. "
        f"backend={backend}."
    )
    return subject, "\n".join(lines)
...
def run_autoresearch(
    *,
    ...
    try:
        if require_env_flag and not os.getenv("OGLAB_AUTORESEARCH_ENABLED"):
            raise GitSafetyError(
                "OGLAB_AUTORESEARCH_ENABLED is not set. This flag exists "
