...
_PERSONALITY: dict[str, dict[str, str]] = {
    # STYLE-SPEC §(d): --glow-mix is the personality glow dial — the
    # percentage fed to color-mix() for every glow wash (--glow-a/--glow-b
    # derived in style.css). Scholarly is dead flat by design: any surface
    # that breaks at --glow-mix: 0% is a bug.
    "technical": {
        "--radius-s": "6px",
        "--radius-m": "10px",
        "--radius-l": "16px",
...
_THEMES: tuple[UITheme, ...] = (
    UITheme(
        id="warm-observatory",
        name="Warm Observatory",
        description=(
            "The default Arail look: a night-observatory lab — warm indigo surfaces, "
            "amber instrument-light for actions, cool cyan starlight for data."
        ),
        # env_value keeps the historical default id so existing LAB_UI_THEME
        # values and worlds with palette_hint "blue-cyan-lab" still resolve
