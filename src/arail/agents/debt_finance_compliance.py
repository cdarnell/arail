...
- ``Provenance.WORLD`` — World-sealed content: a vetted institution's name,
  ...
  scouting finding's ``feed``/``path``/``checked`` metadata.
...
- ``Provenance.SCOUTED_UNVERIFIED`` — a literal substring matched, at tick
  time, out of live-fetched third-party web text by a World-declared
  extraction pattern (``agenda_watch.py``'s "candidate values"). This is
  **not** WORLD provenance, even though it is quoted rather than asserted
  by the agent, and even though it originates from a World's sidecar: it
  was never present at seal time, so it never passed the preflight
  evaluative-language scan that is WORLD provenance's entire justification
  (see below). It gets its own tier specifically so it can be evaluative-
  checked like AGENT text while still being visually distinguishable in
  code from the agent's own words — see ``check_guardrail``'s docstring
  for exactly how it is treated by each check.
...
The evaluative/imperative-language check runs over the concatenation of
``AGENT`` **and** ``SCOUTED_UNVERIFIED`` segments — a ``WORLD`` or
``OPERATOR`` segment can never be evaluative-checked at all, regardless of
what words it happens to contain, because it is never the agent's own
words by construction and it has already passed a seal-time scan.
``SCOUTED_UNVERIFIED`` text has passed neither test — it is quoted, not
asserted, but it is also unaudited live third-party content, so it is
evaluative-checked exactly like ``AGENT`` text and fails closed the same
way. It likewise can never satisfy the institutional-character check's
"is this a vetted name" test (see that check's docstring below) — only
``WORLD``/``OPERATOR`` segments can.
...
**Content that did not exist at seal time is never WORLD provenance, full
stop — no matter how "quoted, not asserted" it is.** A scouting finding's
own ``feed``/``path``/``checked`` fields are WORLD-sealed (``feed`` is the
agenda's own declared URL, ``path`` and ``checked`` are code-generated by
this codebase, never by a third party). A "candidate value" extracted by a
World-declared pattern out of *live-fetched* page text is neither: it is
third-party content that was fetched, matched, and rendered entirely at
tick time, long after any seal-time scan ran. Tagging it ``WORLD`` would
extend the seal-time trust argument to content the seal-time scan never
saw — see ``Provenance.SCOUTED_UNVERIFIED`` above for the tier that exists
specifically so this distinction cannot be erased by a caller reaching for
the nearest "it's quoted" provenance value.
...
class Provenance(enum.Enum):
    ...
    WORLD = "world"
    OPERATOR = "operator"
    SCOUTED_UNVERIFIED = "scouted_unverified"
...
class Segment:
    ...
    @classmethod
    def scouted_unverified(cls, text: str) -> "Segment":
        """A literal substring matched out of live-fetched, third-party web
        text by a World-declared extraction pattern (a scouting finding's
        "candidate value" — see ``agenda_watch.py``). Never ``is_name``:
        a candidate value is never treated as a vetted institution's name,
        regardless of what it contains.

        Deliberately **not** ``Segment.world(...)`` — see the module
        docstring's "content that did not exist at seal time" paragraph for
        why that would be a trust-boundary error, not a shortcut."""
        return cls(text, Provenance.SCOUTED_UNVERIFIED)
...
def check_guardrail(segments: Sequence[Segment]) -> GuardrailResult:
    ...
    **Evaluative/imperative language check:** runs over the concatenation
    of ``AGENT``- **and** ``SCOUTED_UNVERIFIED``-provenance segments (joined
    with a single space, so two adjacent segments can never fuse into a
    word neither of them contains on their own). A ``WORLD`` or
    ``OPERATOR`` segment is never evaluative-checked, no matter what words
    it contains — this is what makes a citation URL or a real institution's
    own marketed name (both historical false-block findings) impossible to
    reintroduce: they are simply never candidates for this check at all.
    ``SCOUTED_UNVERIFIED`` — a scouting finding's candidate value, matched
    out of live-fetched third-party text at tick time and never scanned at
    seal time — is deliberately treated like ``AGENT`` text here, not like
    ``WORLD``: it is quoted rather than asserted, but it has not earned the
    WORLD tier's seal-time-scan justification, so it fails closed the same
    way ungrounded agent prose does.
    ...
    **Institutional-character check:** runs over the full, in-order
    ...
    2. The trigger's own segment is ``AGENT``- or ``SCOUTED_UNVERIFIED``-
       provenance. Then the claim is legitimate only if one of its
       immediate neighbouring segments is *specifically* the segment the
       caller tagged ``is_name=True`` at construction — the segment
       carrying the institution's actual name — and that segment carries
       ``WORLD`` or ``OPERATOR`` provenance. ``SCOUTED_UNVERIFIED`` gets no
       special treatment here either: a candidate value can never itself
       stand in as the vetted-name neighbour (``Segment.scouted_unverified``
       never sets ``is_name``), so a candidate value containing an
       institutional-character phrase is exactly as unable to self-vouch
       as raw agent prose is.
    ...
    must specifically be tagged as carrying the institution's own name, and
    that neighbour's own provenance must be ``WORLD`` or ``OPERATOR`` — a
    name that exists only inside ``AGENT``- or ``SCOUTED_UNVERIFIED``-
    provenance text — invented, altered, or merely fetched off a live page
    — can never satisfy this, because there is no string it could be
    altered *to* that would change its segment's provenance or name tag.
    """
    checkable_text = " ".join(
        s.text for s in segments
        if s.provenance in (Provenance.AGENT, Provenance.SCOUTED_UNVERIFIED)
    )
    if _EVALUATIVE_RE.search(checkable_text):
        return GuardrailResult(ok=False, reason=REASON_EVALUATIVE)

    def _is_name_voucher(s: Segment) -> bool:
        return s.provenance in (Provenance.WORLD, Provenance.OPERATOR) and s.is_name

    for idx, seg in enumerate(segments):
        for match in _INSTITUTIONAL_CHARACTER_RE.finditer(seg.text):
            if seg.provenance in (Provenance.WORLD, Provenance.OPERATOR):
