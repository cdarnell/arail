...
def _vetted_institutions(terms: List[Dict[str, Any]]) -> List[VettedInstitution]:
    """Institution-category terms — the only names Debt Advisor may pair
    with "credit union"/"nonprofit" language (ARCHITECTURE.md §3.3/§4.3)."""
    out = []
    for t in terms:
        if t.get("category") == "institutions" and t.get("source"):
            out.append(VettedInstitution(
                name=str(t.get("term") or t.get("slug") or ""),
                verification_source=str(t.get("source")),
            ))
    return out
...
def _build_output(bundle_dir: Path, terms: List[Dict[str, Any]],
    ...
    if vetted:
        for v in vetted:
            # Every name and source URL below is inserted verbatim from the
            # term's own structured fields — never generated by the model.
            lines.append(
