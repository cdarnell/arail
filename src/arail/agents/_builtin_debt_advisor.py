...
def _vetted_institutions(terms: List[Dict[str, Any]]) -> List[VettedInstitution]:
    ...
    for t in terms:
        ...
        if t.get("category") == "institutions" and institution_type and t.get("verification_source"):
...
def _build_output(bundle_dir: Path, terms: List[Dict[str, Any]],
    ...
    lines.append("## Vetted institutions (from this World's sourced terms)\n")
    if vetted:
        for v in vetted:
            # Every name, character label, and verification-source URL below
            # is inserted verbatim from the term's own structured fields —
            # never a hardcoded string, never generated by the model. Each
            # institution's actual institution_type is what gets printed, so
            # a credit-counseling agency is never mislabeled as a credit
            # union (BLOCK-2).
