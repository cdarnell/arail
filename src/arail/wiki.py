"""Arail Wiki — the documentation-as-code compiler.
...
- Extends ``arail.pkb`` rather than replacing it; ``pkm.compile_index``
...
- Frontmatter is optional on user pages, required on auto-generated
  pages from :mod:`arail.docgen`.
- Wikilinks follow Obsidian syntax: ``[[target]]``, ``[[target|alias]]``,
  ``[[target#heading]]``.

Exposes a CLI entry point: ``python -m arail.wiki build``.
"""
...
def _is_low_signal_experiment_page(pkb_root: Path, md: Path, text: str) -> bool:
    """Suppress old autogenerated experiment stubs from wiki graph/page index."""
    try:
...
def build_link_graph(
    ...
    if include_semantic and pkb_root is not None:
        try:
            from arail.wiki_vectors import semantic_edges_for_pages
...
def compile_wiki(pkb_root: Optional[Path] = None,
    ...
    if repo_root is not None:
        try:
            # importlib avoids a static import cycle with arail.docgen
            # (which arrives in Phase 3 and itself imports from this module).
            import importlib
            docgen = importlib.import_module("arail.docgen")
...
def _cli() -> None:
    import argparse
    parser = argparse.ArgumentParser(prog="python -m arail.wiki")
