"""Arail docgen — turn the repo's own source into wiki pages.

Walks a repo root and emits markdown pages into
``{pkb_root}/compiled/docs/`` for:

- **Python modules** (``src/arail/**/*.py``) — parsed via ``ast`` to
...
- **Shell scripts** (``scripts/*.sh``, plus the root ``arail``
...
def _guide_to_markdown(source_path: Path, rel_to_repo: str) -> str:
    ...
    from arail.wiki import parse_frontmatter
...
def generate_all(repo_root: Path, pkb_root: Path) -> dict[str, int]:
    ...
    # 1. Python modules under src/arail
    py_dir = repo_root / "src" / "arail"
    if py_dir.is_dir():
