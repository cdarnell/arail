#!/usr/bin/env bash
# blueprint.sh — implementation of `./arail blueprint <subcommand>`.
#
# Subcommands:
...
#   The default ARAIL lab — what `./arail setup` provisions — stays
...
cmd_show() {
    local id="${1:-}"
    [[ -n "$id" ]] || die "Usage: ./arail blueprint show <id>"
    local path="$BLUEPRINTS_DIR/$id/blueprint.toml"
    [[ -f "$path" ]] || die "blueprint not found: $id (try: ./arail blueprint list)"
    echo "── $path ──"
    cat "$path"
}
...
cmd_create() {
    ...
    [[ -n "$instance" ]] || die "Usage: ./arail blueprint create <instance> --from <blueprint-id>"
    ...
    [[ -f "$bp_path" ]] || die "blueprint not found: $blueprint_id (try: ./arail blueprint list)"

    local idir="$INSTANCES_DIR/$instance"
    [[ ! -d "$idir" ]] || die "instance already exists: $idir (try: ./arail blueprint apply $instance)"
    ...
    say ""
    say "Next: ./arail blueprint apply $instance   (re-render after editing)"
}
...
cmd_apply() {
    require_python_311
    local instance="${1:-}"
    [[ -n "$instance" ]] || die "Usage: ./arail blueprint apply <instance>"
    local idir="$INSTANCES_DIR/$instance"
    [[ -d "$idir" ]] || die "instance not found: $instance (try: ./arail blueprint create $instance --from <id>)"
...
cmd_destroy() {
    local instance="${1:-}"
    [[ -n "$instance" ]] || die "Usage: ./arail blueprint destroy <instance>"
    local idir="$INSTANCES_DIR/$instance"
...
env_lines = [
    f"# {instance} — generated by ./arail blueprint apply {instance}",
...
cmd_help() {
  ...
  Examples:
    ./arail blueprint list
    ./arail blueprint catalog
    ./arail blueprint create research --from autoresearch
    ./arail blueprint apply research
    ./arail blueprint destroy research
